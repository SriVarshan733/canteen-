...
if(isset($_POST['submit'])){
   ...
   if(!empty($name)){
      $select_name = $conn->prepare("SELECT * FROM `admin03` WHERE name = ?");
      $select_name->execute([$name]);
      if($select_name->rowCount() > 0){
         $message[] = 'username already taken!';
      }else{
         $update_name = $conn->prepare("UPDATE `admin03` SET name = ? WHERE id = ?");
         $update_name->execute([$name, $admin_id]);
      }
   }

   $empty_pass = 'da39a3ee5e6b4b0d3255bfef95601890afd80709';
   $select_old_pass = $conn->prepare("SELECT password FROM `admin03` WHERE id = ?");
   ...
   if($old_pass != $empty_pass){
      if($old_pass != $prev_pass){
         ...
         if($new_pass != $empty_pass){
            $update_pass = $conn->prepare("UPDATE `admin03` SET password = ? WHERE id = ?");
