...
if(isset($_POST['submit'])){
   ...
   if(!empty($name)){
      $select_name = $conn->prepare("SELECT * FROM `admin02` WHERE name = ?");
   ...
   $select_old_pass = $conn->prepare("SELECT password FROM `admin02` WHERE id = ?");
   ...
   if($old_pass != $empty_pass){
      if($old_pass != $prev_pass){
         ...
         if($new_pass != $empty_pass){
            $update_pass = $conn->prepare("UPDATE `admin02` SET password = ? WHERE id = ?");
